...
class PHPThumb {
	...
	/**
	 * Class constructor
	 * @param $imageCache - Set the cache folder destination for thumbs created by phpThumb view helper, the default is '/ Imagecache /'
	 */	
	...
	public function Createthumb($filename="unknown",$output="screen") {
		...
		} elseif (!is_file($this->getImagemThumb())) {
			$this->image=$filename;
			$this->thumbmaker();
			if ($output=="file") {$this->savethumb();} else {$this->displaythumb();}
		} else {
		    echo $this->getImagemThumbCache();
		}
	...
	private function getImagemThumb()
	{
	    return $this->root . $this->imageCache . $this->imageName;
	}
	
	private function getImagemThumbCache()
	{
	    return $this->imageCache . $this->imageName;
	}
	...
	private function displaythumb() {
		...
		switch($this->size['mime']) {
			case "image/gif":
				imagegif($this->thumb, $this->getImagemThumb(), $this->Quality);
				echo $this->getImagemThumbCache();
				break;
			case "image/jpeg":
				imagejpeg($this->thumb, $this->getImagemThumb(), $this->Quality);
				echo $this->getImagemThumbCache();
				break;
			case "image/png":
				imagejpng($this->thumb, $this->getImagemThumb(), $this->Quality);
				echo $this->getImagemThumbCache();
				break;
		}
		imagedestroy($this->im);
